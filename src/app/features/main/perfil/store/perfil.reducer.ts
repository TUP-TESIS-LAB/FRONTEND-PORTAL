...
export const perfilReducer = createReducer(
  ...
  on(A.updateProfile, s => ({ ...s, saving: true, profileSaved: false, error: null })),
  on(A.updateProfileSuccess, (s, { profile }) => ({ ...s, saving: false, user: profile, profileSaved: true })),
  on(A.updateProfileFailure, (s, { error }) => ({ ...s, saving: false, error })),
  on(A.profileSavedHandled, s => ({ ...s, profileSaved: false })),
